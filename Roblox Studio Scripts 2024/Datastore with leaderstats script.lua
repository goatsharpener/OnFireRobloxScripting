...
local function saveData(player) --Create out own function for saving data
	local tableToSave = { --Creates a table for our values
		player.leaderstats.Points.Value; --Saves the first value
...
game.Players.PlayerAdded:Connect(function(player) --When a player joins the game
	...
	points.Name = "Points" --Sets IntValue name to "Points" (If you change this make sure you change all of them)
	points.Parent = leaderstats --Puts the IntValue under the "leaderstats" folder
	points.Value = 0 --Gives the IntValue Value to start off with

	local wins = Instance.new("IntValue") --Creates an IntValue
	wins.Name = "Wins" --Sets IntValue name to "Wins" (If you change this make sure you change all of them)
	wins.Parent = leaderstats --Puts the IntValue under the "leaderstats" folder
	wins.Value = 0 --Gives the IntValue Value to start off with
